...
viewer_app = ScriptBotsViewer(0, 4, 69, 32, 1375, 768)
...
def train_step(sim_mgr):
    ...
    random_shit = torch.zeros(10).cuda()

    species_counts = sim_mgr.species_count_tensor().to_torch()

    #species_end_offsets = species_counts.sum(dim=0).cumsum(dim=0)
    species_end_offsets_tmp = species_counts.sum(dim=0)
    species_end_offsets = species_end_offsets_tmp.cumsum(dim=0)
